  ...
  useEffect(() => {
    const fetchCertificate = async () => {
      ...
      try {
        ...
        const certificateData = { id: certificateSnapshot.docs[0].id, ...certificateSnapshot.docs[0].data() };
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <h4 style={{ margin: '0 0 10px 0', color: '#333' }}>Certificate Details</h4>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
          <div><strong>Course:</strong> {course.title}</div>
          <div><strong>Completed:</strong> {formatDate(certificate.completedAt)}</div>
