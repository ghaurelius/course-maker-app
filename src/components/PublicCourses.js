...
function PublicCourses() {
  ...
  }, [currentUser, isEnrolled]); // Dependencies for useCallback

  useEffect(() => {
    fetchPublishedCourses();
  }, [fetchPublishedCourses]); // Now includes fetchPublishedCourses

  // Filter change handler for FilterPanel
  const handleFilterChange = (fieldName, value) => {
    console.log('Filter change:', fieldName, value); // Debug line
  ...
  // Remove the unused handleSearchChange function
  ...
  const filteredCourses = courses.filter(course => {
    ...
    // Duration filter - only apply if course has duration field
    const matchesDuration = !filters.duration || !course.duration || course.duration === filters.duration;
    
    // Free only filter - only apply if course has price/isFree fields
    const matchesFree = !filters.freeOnly || 
      (course.price !== undefined && course.price === 0) || 
      (course.isFree !== undefined && course.isFree === true) ||
      (course.price === undefined && course.isFree === undefined); // If no price info, show all
    
    // Rating filter - only apply if course has rating field
