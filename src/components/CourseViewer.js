...
function CourseViewer() {
  ...
  const { updateLessonProgress, getEnrollmentDetails, enrollInCourse } = useEnrollment();
        ...
        if (courseDoc.exists()) {
          ...
          if (currentUser) {
            ...
            if (enrollmentData) {
              ...
              setCompletedLessons(enrollmentData.progress?.completedLessons || []);
              setCurrentModuleIndex(enrollmentData.progress?.currentModule || 0);
              setCurrentLessonIndex(enrollmentData.progress?.currentLesson || 0);
            }
          }
        } else {
  ...
  const handleLessonComplete = async (moduleIndex, lessonIndex) => {
    ...
    if (!completedLessons.includes(lessonId)) {
      const success = await updateLessonProgress(courseId, moduleIndex, lessonIndex);
      if (success) {
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: '4px',
                    cursor: 'pointer',
                    backgroundColor: isActive ? '#007bff' : (isCompleted ? '#d4edda' : 'white'),
                    color: isActive ? 'white' : (isCompleted ? '#155724' : '#333'),
                    border: '1px solid ' + (isActive ? '#007bff' : (isCompleted ? '#c3e6cb' : '#ddd')),
                    fontSize: '14px',
                    display: 'flex',
                    alignItems: 'center',
                    gap: '8px'
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        {!enrollment && currentUser && (
          <div style={{
            ...
            <h4 style={{ margin: '0 0 10px 0', color: '#856404' }}>Enroll to Track Progress</h4>
        ...
        {!currentUser && (
          <div style={{
            ...
            <h4 style={{ margin: '0 0 10px 0', color: '#0c5460' }}>Login to Enroll</h4>
          ...
          <div>
            <div style={{ marginBottom: '20px' }}>
              ...
              <div style={{ 
                ...
                {currentLesson.duration && <span>⏱️ {currentLesson.duration}</span>}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    fontSize: '14px',
                    opacity: (currentModuleIndex === 0 && currentLessonIndex === 0) ? 0.5 : 1
                  }}
                >
                  ← Previous
                </button>
                ...
                <button
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    fontSize: '14px',
                    opacity: (currentModuleIndex === course.modules.length - 1 && 
                             currentLessonIndex === currentModule.lessons.length - 1) ? 0.5 : 1
                  }}
                >
                  Next →
                </button>
              ...
              {enrollment && (
                <button
                  onClick={() => handleLessonComplete(currentModuleIndex, currentLessonIndex)}
                  disabled={isLessonCompleted(currentModuleIndex, currentLessonIndex)}
                  style={{
                    backgroundColor: isLessonCompleted(currentModuleIndex, currentLessonIndex) 
                      ? '#28a745' : '#ffc107',
                    color: 'white',
                    border: 'none',
                    padding: '10px 20px',
